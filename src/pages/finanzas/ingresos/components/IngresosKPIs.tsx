...
import { MoneyIcon, GraduationCapIcon, AiFolderIcon, SchoolIcon } from "@hugeicons/core-free-icons"
...
export function IngresosKPIs({ totales }: { totales: KPIData }) {
  const items = [
    { label: "Total ingresado", value: totales.total, previo: totales.previo_total, color: "oklch(0.55 0.15 150)", bg: "oklch(0.55 0.15 150 / 0.1)", icon: MoneyIcon },
      ...
      {items.map((item, i) => (
        <div key={i} className="rounded-2xl border bg-white p-4 relative overflow-hidden" style={{ borderColor: BORDER }}>
          ...
          <div className="flex items-baseline gap-1 relative z-10">
            <p className="text-xl font-black" style={{ color: i === 0 ? item.color : CHARCOAL }}>
