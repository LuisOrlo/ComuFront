      ...
      <div className="rounded-2xl border bg-white p-5 space-y-3" style={{ borderColor: BORDER }}>
        <div className="flex items-center gap-3">
          <div className="size-12 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: "oklch(0.95 0.01 45)" }}>
            <HugeiconsIcon icon={AiFolderIcon} size={22} style={{ color: COLORS.ACCENT }} />
          </div>
          <div>
          ...
          <tbody className="divide-y" style={{ borderColor: BORDER }}>
            {(!entries || entries.length === 0) ? (
              ...
              entries.map((entry: any, i: number) => {
                ...
                return (
                  <tr key={cuentaId || entry.id || `idx-${i}`} className="hover:bg-gray-50/60 transition-colors">
                    ...
                    <td className="p-3 border-r text-xs font-medium" style={{ borderColor: BORDER, color: CHARCOAL }}>${montoTotal.toLocaleString()}</td>
                    <td className="p-3 border-r text-xs font-medium text-green-600" style={{ borderColor: BORDER }}>${montoAbonado.toLocaleString()}</td>
                    <td className="p-3 border-r text-xs font-medium" style={{ borderColor: BORDER, color: montoSaldo > 0 ? "#dc2626" : undefined }}>${montoSaldo.toLocaleString()}</td>
                    <td className="p-3 border-r" style={{ borderColor: BORDER }}>
                      <span className={cn(
