...
export function StudentCreateModal({ open, onOpenChange, onCreated }: StudentCreateModalProps) {
  ...
  const resetForm = () => setForm({ nombres: "", apellidos: "", cedula: "", correo: "", celular: "", ciudad_id: "", fecha_nacimiento: "" })
        ...
        <Dialog.Content className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-[2rem] w-full max-w-lg my-auto overflow-hidden shadow-2xl">
            ...
            <form onSubmit={handleCreate} className="p-8 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-5">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Cedula</label>
                  <input type="text" value={form.cedula} onChange={e => setForm({ ...form, cedula: e.target.value })} placeholder="1234567890" className="w-full px-4 py-3 border border-gray-200 rounded-2xl text-sm outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all" />
                </div>
                <div className="space-y-1.5">
