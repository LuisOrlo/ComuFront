...
export function ReservaBatchForm({
    ...
    Promise.all([
      ...
      .then(([pkgs, people]) => {
        ...
        setReservas((rs) =>
          rs.map((r) => ({
            ...r,
            paquete_id: r.paquete_id || initialPaqueteId || pkgs[0]?.id || "",
  ...
  }, [initialPaqueteId, initialDate, initialStart, initialEnd])
  ...
  const duplicateDraft = (r: Draft) => {
    const newDraft: Draft = {
      ...
      titulo: r.titulo ? `${r.titulo} (Sesión siguiente)` : "",
      notas: r.notas,
      descuento: "",
      motivo_descuento: "",
  ...
  const validate = () => {
    const next = reservas.map((r) => {
      ...
      if (!r.paquete_id) errors.paquete_id = "Seleccione un paquete"
    ...
    for (let i = 0; i < next.length; i++) {
      for (let j = i + 1; j < next.length; j++) {
        if (checkOverlap(next[i], next[j])) {
          next[j].errors.hora_inicio = `Conflicto con la Sesión #${i + 1} (mismo horario en cabina)`
        }
      }
    }
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!cliente) {
      toast.error("Debes seleccionar un cliente o responsable para la reserva")
      return
    }
    if (!validate()) {
      toast.error("Revisa los campos obligatorios o conflictos de horario")
      return
    }

    setSaving(true)
    try {
      await podcastService.createReservasBatch({
        ...
        reservas: reservas.map((r) => {
          ...
          return {
            ...
            motivo_descuento: r.motivo_descuento?.trim() || null,
            estado: "pendiente",
      ...
      setReservas(next)
      toast.error(data?.message || "No se pudieron registrar las reservas de podcast")
    } finally {
      setSaving(false)
    }
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-7 flex flex-col gap-6">
          ...
          <div className="flex items-center gap-3.5 min-w-0">
            ...
            <div className="min-w-0">
              <span className="text-[11px] uppercase tracking-wider text-slate-500 font-semibold">
                Cabina de Podcast / Nueva Reserva por Lote
              </span>
              <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight truncate">
                Reserva de Cabina de Podcast (Individual o por Lote)
              </h1>
            </div>
          </div>
        ...
        <form onSubmit={submit} className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
                ...
                <div className="relative" ref={clienteRef}>
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Ingresa al menos 2 letras para buscar estudiantes, instructores o clientes externos..."
                  ...
                  <p className="text-[11px] text-slate-400 mt-1.5 flex items-center gap-1.5">
                    ...
                    />
                    <span>Escribe al menos 2 letras para iniciar la búsqueda de clientes o alumnos.</span>
                  </p>
            ...
            {/* Card 2: Lista de Sesiones Programadas de Podcast */}
            <div className="space-y-4">
              <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h2 className="text-sm font-bold text-slate-900 tracking-tight">
                      Sesiones de Cabina Programadas
                    </h2>
                    <p className="text-xs text-slate-500">
                      Configura el paquete, fecha, horario y staff de cada grabación
                    </p>
                  </div>
                </div>
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between pb-3 border-b border-slate-100 gap-2">
                        <div className="flex items-center gap-2.5 flex-wrap">
                          ...
                          {r.titulo && (
                            <span className="text-[11px] font-medium text-slate-500 italic truncate max-w-[180px]">
                              "{r.titulo}"
                            </span>
                          )}
                        </div>
                      ...
                      {/* Error Banner si hay conflicto o error de validación */}
                      {hasConflict && (
                      ...
                      {/* Cuadrícula de Campos de la Sesión */}
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                        ...
                        <div className="space-y-1 sm:col-span-2">
                          <label className="text-[11px] font-bold uppercase tracking-wider text-slate-600 flex items-center gap-1">
                            ...
                            <span>
                              Paquete de Podcast <span className="text-[#fd761a]">*</span>
                            </span>
                          </label>
                          <select
                            value={r.paquete_id}
                            onChange={(e) => update(r.id, { paquete_id: e.target.value })}
                            className={cn(
                        ...
                        <div className="space-y-1 sm:col-span-2">
                          ...
                          <input
                            type="text"
                            placeholder="Ej. Entrevista a emprendedores #12..."
                            value={r.titulo}
                            onChange={(e) => update(r.id, { titulo: e.target.value })}
                            className="w-full h-10 px-3 rounded-xl border border-slate-200 bg-white text-xs font-medium text-slate-800 outline-none focus:border-[#fd761a] focus:ring-2 focus:ring-[#fd761a]/20 transition-all placeholder:text-slate-400"
                          />
                        </div>
                        ...
                        <div className="space-y-1 sm:col-span-2">
                          <label className="text-[11px] font-bold uppercase tracking-wider text-slate-600 flex items-center gap-1">
                            ...
                            <span>
                              Fecha de Grabación <span className="text-[#fd761a]">*</span>
                            </span>
                          </label>
                          <input
                            type="date"
                        ...
                        <div className="space-y-1 sm:col-span-4">
                          <label className="text-[11px] font-bold uppercase tracking-wider text-slate-600 flex items-center gap-1">
                            ...
                            <span>Notas de Grabación / Requerimientos</span>
                          </label>
                          <input
                            type="text"
                            placeholder="Requerimientos técnicos, número de micrófonos, grabación de video adicional..."
                            value={r.notas}
                            onChange={(e) => update(r.id, { notas: e.target.value })}
                            className="w-full h-10 px-3 rounded-xl border border-slate-200 bg-white text-xs text-slate-800 outline-none focus:border-[#fd761a] focus:ring-2 focus:ring-[#fd761a]/20 transition-all placeholder:text-slate-400"
                          />
                        </div>
                      </div>

                      {/* Asignaciones de Personal (Staff) */}
                      <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/70 space-y-2.5">
                        <div className="flex items-center justify-between">
                          <label className="text-[11px] font-bold uppercase tracking-wider text-slate-700 flex items-center gap-1.5">
                            <Users size={13} className="text-slate-400" />
                            <span>Personal Asignado en Cabina</span>
                            <span className="text-[10px] text-slate-400 font-normal">
                              ({r.asignaciones.length} asignado{r.asignaciones.length !== 1 ? "s" : ""})
                            </span>
                          </label>
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                update(r.id, {
                                  asignaciones: [
                                    ...r.asignaciones,
                                    { persona_id: e.target.value, rol: "Técnico de audio" },
                                  ],
                                })
                              }}
                        ...
                        {/* Chips con personas asignadas */}
                        {r.asignaciones.length > 0 ? (
                          <div className="flex flex-wrap gap-2 pt-1">
                            {r.asignaciones.map((asig, i) => {
                              const person = personas.find((x) => x.id === asig.persona_id)
                              return (
                                <div
                                  key={asig.persona_id}
                                  className="inline-flex items-center gap-2 pl-2.5 pr-1.5 py-1 rounded-xl bg-white border border-slate-200 shadow-2xs text-xs font-medium text-slate-800"
                                >
                                  ...
                                  <input
                                    type="text"
                                    value={asig.rol}
                                    placeholder="Rol (ej. Sonido)"
                                    onChange={(e) => {
                                      const nextAsig = [...r.asignaciones]
                                      nextAsig[i] = { ...nextAsig[i], rol: e.target.value }
                                      update(r.id, { asignaciones: nextAsig })
                                    }}
                                    className="w-24 text-[11px] font-semibold text-purple-700 bg-purple-50 px-1.5 py-0.5 rounded outline-none border border-transparent focus:border-purple-200 placeholder:text-purple-300"
                                  />
                                  <button
                                    type="button"
                                    onClick={() =>
                                      update(r.id, {
                                        asignaciones: r.asignaciones.filter(
                                          (x) => x.persona_id !== asig.persona_id
                                        ),
                                      })
                                    }
                                    className="size-5 rounded-full hover:bg-slate-100 text-slate-400 hover:text-red-600 flex items-center justify-center transition-colors cursor-pointer"
                          ...
                          <p className="text-[11px] text-slate-400 italic">
                            Sin personal asignado a esta sesión de cabina.
                          </p>
                        )}
                      </div>
              ...
              >
                <Plus size={16} />
                <span>Agregar otra sesión o fecha de podcast al lote</span>
              </button>
            </div>
          </div>
            ...
            <div className="rounded-xl shadow-xs bg-white border border-slate-200/90 p-6 space-y-5">
              ...
              <div className="space-y-2 pt-2 border-t border-slate-100 text-xs text-slate-600">
                <div className="flex justify-between">
                  <span>Horas de cabina reservadas:</span>
                  <span className="font-bold text-slate-900">{totals.sumHours} hrs</span>
                </div>
                <div className="flex justify-between">
                ...
                >
                  {saving ? (
                    <>
                      ...
                      <span>Registrando {reservas.length} reservas...</span>
                    </>
                  ) : (
                    <span>Confirmar y Registrar Reservas</span>
                  )}
                </button>
        ...
        <NuevoClienteModal
          ...
          onCreated={(newClient) => {
            ...
            toast.success("Cliente creado y asignado como responsable")
          }}
        />
      </div>
    </div>
