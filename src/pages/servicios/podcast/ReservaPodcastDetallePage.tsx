...
export function ReservaPodcastDetallePage() {
  ...
  const backHref = `/servicios/podcast/historial${(() => { const p = new URLSearchParams(); const e = searchParams.get("estado"); const s = searchParams.get("search"); if (e) p.set("estado", e); if (s) p.set("search", s); const qs = p.toString(); return qs ? `?${qs}` : "" })()}`
  ...
  useEffect(() => {
    ...
    podcastService.getReserva(id)
      .then(setReserva)
      .catch(() => { toast.error("Error al cargar reserva"); navigate(backHref) })
      .finally(() => setLoading(false))
  }, [id, navigate, backHref])

  const handleTapCompleted = async () => {
    if (!reserva) return
    setSaving(true)
    try {
      await podcastService.cambiarEstado(reserva.id, "completado")
  ...
  const isOverdue = (reserva.estado === "pendiente" || reserva.estado === "confirmado") && new Date(`${reserva.fecha_reserva}T${reserva.hora_fin}`) < new Date()
  const getCliente = () => {
    if (reserva.persona) return `${reserva.persona.nombres} ${reserva.persona.apellidos}`
    if (reserva.cliente_externo) return `${reserva.cliente_externo.nombres} ${reserva.cliente_externo.apellidos || ""}`
    return "—"
  }
  const clienteCedula = reserva.cliente_externo?.cedula
  const clienteEmail = reserva.persona?.correo || reserva.cliente_externo?.correo
        ...
        <div className="max-w-4xl mx-auto px-6 lg:px-8 py-5">
          <div className="flex items-center gap-4">
            ...
            <select
              ...
              onChange={e => reserva && podcastService.cambiarEstado(reserva.id, e.target.value).then(r => setReserva(r)).catch(() => toast.error("Error"))}
              disabled={saving}
        ...
        <div className="max-w-4xl mx-auto px-6 lg:px-8 py-6 space-y-5">
          ...
          <div className="bg-white rounded-2xl border p-5 space-y-3" style={{ borderColor: COLORS.BORDER_SUBTLE }}>
            <h3 className="text-xs font-bold flex items-center gap-2" style={{ color: COLORS.CHARCOAL }}>
              <HugeiconsIcon icon={UserIcon} size={14} className="text-indigo-500" /> Cliente
            </h3>
          ...
          {reserva.asignaciones && reserva.asignaciones.length > 0 && (
            <div className="bg-white rounded-2xl border p-5 space-y-3" style={{ borderColor: COLORS.BORDER_SUBTLE }}>
              <h3 className="text-xs font-bold flex items-center gap-2" style={{ color: COLORS.CHARCOAL }}>
                <HugeiconsIcon icon={UserGroupIcon} size={14} className="text-violet-500" /> Personal a cargo
              </h3>
              <div className="space-y-2">
                {reserva.asignaciones.map(asig => (
                  <div key={asig.id} className="flex items-center gap-2.5">
                    <div className="size-8 rounded-lg bg-violet-100 flex items-center justify-center shrink-0">
                      <HugeiconsIcon icon={UserIcon} size={14} className="text-violet-500" />
                    </div>
                    <div>
                      <p className="text-xs font-bold" style={{ color: COLORS.CHARCOAL }}>
                        {asig.persona?.nombres} {asig.persona?.apellidos}
                      </p>
                      {asig.rol && <p className="text-[10px] opacity-40">{asig.rol}</p>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          {reserva.titulo && (
            <div className="bg-white rounded-2xl border p-5" style={{ borderColor: COLORS.BORDER_SUBTLE }}>
              <p className="text-[9px] font-bold uppercase tracking-widest opacity-40 flex items-center gap-1.5 mb-2">
                <HugeiconsIcon icon={Microphone} size={11} /> Título del episodio
              </p>
              <p className="text-sm font-medium" style={{ color: COLORS.CHARCOAL }}>{reserva.titulo}</p>
            </div>
          )}
          ...
          {reserva.notas && (
            <div className="bg-white rounded-2xl border p-5" style={{ borderColor: COLORS.BORDER_SUBTLE }}>
              <p className="text-[9px] font-bold uppercase tracking-widest opacity-40 flex items-center gap-1.5 mb-2">
                <HugeiconsIcon icon={InformationCircleIcon} size={11} /> Notas
              </p>
              <p className="text-xs opacity-60">{reserva.notas}</p>
            </div>
          )}

          <div className="flex items-center gap-3 pt-2 pb-4">
            {(reserva.estado === "pendiente" || reserva.estado === "confirmado" || reserva.estado === "en_progreso") && (
              <button onClick={handleTapCompleted} disabled={saving}
            ...
            {!reserva.pago_registrado && (
              <button onClick={() => navigate(`/finanzas/pagos/cuentas/servicios/pago/${reserva.id}`, { state: { tipo: "podcast", servicioId: reserva.id, nombre: getCliente(), montoTotal: Number(reserva.precio_total) || 0, montoSaldo: Number(reserva.precio_total) || 0, nombreServicio: reserva.titulo || reserva.paquete?.nombre || "Podcast" } })} className="flex-1 py-3 rounded-xl text-sm font-bold border transition-all hover:bg-gray-50 active:scale-[0.98]"
            ...
            {reserva.estado === "completado" && (
              <p className="text-sm font-medium py-3 opacity-40" style={{ color: COLORS.CHARCOAL }}>Esta reserva ya fue completada</p>
            )}
          </div>
        </div>
      </div>
