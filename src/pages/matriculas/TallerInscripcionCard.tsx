...
export function TallerInscripcionCard({ ins, isExpanded, puedeVerificar, editTallerField, editTallerVal, savingTallerEdit, onToggle, onEdit, onChange, onSave, onCancel, onApprove, onReject, onExpandImage, onAfterMutate }: {
  ...
  onSave: () => void; onCancel: () => void;   onApprove: (monto: number, tipoPago: string, metodoPago: string, precioAjustado: number, motivoAjuste: string) => void; onReject: () => void
      ...
      {isExpanded && (
        <div className="border-t px-5 py-5 space-y-5" style={{ borderColor: COLORS.BORDER_SUBTLE, background: "oklch(0.985 0 0)" }}>
            ...
            <div className="space-y-3">
              <SubCategory title="Información Personal" color={AZUL}>
                <EF icon={UserIcon} label="Nombres" field="nombres" data={ins}
                  ...
                  onEdit={onEdit} onChange={onChange} onSave={onSave} onCancel={onCancel} saving={savingTallerEdit} groupColor={AZUL} />
                ...
                <EF icon={Calendar03Icon} label="Fecha Nacimiento" field="fecha_nacimiento" data={ins}
                  editField={editTallerField} editVal={editTallerVal}
                  onEdit={onEdit} onChange={onChange} onSave={onSave} onCancel={onCancel} saving={savingTallerEdit} inputType="date" groupColor={AZUL} />
              ...
              <SubCategory title="Contacto" color={VERDE}>
                <EF icon={MailIcon} label="Correo" field="correo" data={ins}
                  editField={editTallerField} editVal={editTallerVal}
                  onEdit={onEdit} onChange={onChange} onSave={onSave} onCancel={onCancel} saving={savingTallerEdit} groupColor={VERDE} />
              ...
              <SubCategory title="Ubicación" color={PURPURA}>
                <EF icon={Location01Icon} label="Dirección" field="direccion" data={ins}
                  editField={editTallerField} editVal={editTallerVal}
                  onEdit={onEdit} onChange={onChange} onSave={onSave} onCancel={onCancel} saving={savingTallerEdit} groupColor={PURPURA} />
          ...
          <Section title="Taller" icon={BookOpenIcon}>
            <div className="p-3 rounded-xl space-y-2" style={{ backgroundColor: `color-mix(in srgb, ${COLORS.ACCENT} 6%, transparent)`, borderLeft: `3px solid ${COLORS.ACCENT}` }}>
              ...
              <InfoItem icon={PaymentIcon} label="Precio" value={ins.taller?.precio ? `$${Number(ins.taller.precio).toFixed(2)}` : "—"} bold groupColor={COLORS.ACCENT} />
            </div>
          </Section>
          ...
          {puedeVerificar && (
            <div className="flex gap-3 pt-3">
              ...
              <button onClick={() => onApprove(tallerMonto, tipoPagoActual, ins.metodo_pago || "efectivo", precioEfectivo, motivoAjuste)}
